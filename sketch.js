...
is_clicked = false
var q = 1;
var begin,welcome,welcome2, inputBox;
...
function setup() {
 ...
 // timer();
  ...
  timerText.position(width/2-100,50);
...
function question3(){
  ...
  q1.hide();
  q1 = createElement("h2");
  q1.position(width/2-200,height/2-75);
  q1.class("textColor")
...
function question4(){
  ...
  q1.hide();
  q1 = createElement("h2");
  q1.position(width/2-350,height/2-75);
  q1.class("textColor")
...
function question7(){
  ...
  q1.hide();
  q1 = createElement("h2");
  q1.position(width/2-525,height/2-75);
  q1.class("textColor")
...
function question10(){
  ...
  button3.size(250,150);
  button3.class("buttonStyle");
  button3.mouseClicked(winnerBox);
...
function winnerBox(){
  swal({
    ...
    imageUrl:
      "https://transform-mpi.com/wp-content/uploads/2017/12/TRAINING-TENTANG-BUILDING-MOTIVATION-TO-WIN-BMW.png",
    imageSize: "200x200",
    confirmButtonText: "End the quiz",
    },(confirmed)=>{
      if(confirmed == true){
        location.reload();
      }
...
function loseBox(){
  swal({
    ...
    imageUrl:
      "https://upload.wikimedia.org/wikipedia/commons/c/c9/Sorry-you-just-lost-the-game.jpg",
    imageSize: "200x200",
    confirmButtonText: "End the quiz"
  });
...
function firstPage(){
  ...
  welcome.position(width/2-400,height/2-75);
  welcome.html(`Welcome to my game <br> You will have to complete all 10 questions in order to beat this quiz`)
  
  inputBox = createInput().attribute("placeholder", "Enter your name");
  inputBox.position(width/2-200,height/2+20);
  inputBox.size(400,50);
  inputBox.class("placeHolder");
  inputBox.class("customInput");

  //welcome2 = createElement("h2");
  //welcome2.position(100,100);
  //welcome2.html("Lets Begin");

  begin = createButton("Ok");
  begin.position(width/2-20,height/2+100);
  begin.size(60,40);
  begin.mouseClicked(question1);
  begin.class("customButton")

}

function position(){
  quizMaster.x = width-100;
  quizMaster.y = height-135;
  quizMaster.scale = 1
}

function changePos(){
  quizMaster.x = width-100;
  quizMaster.y = height-100;
}
